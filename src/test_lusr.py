...
def evaluate(env, agent, video, num_episodes, eval_mode, steps):
	episode_rewards = []
	for i in tqdm(range(num_episodes)):
		...
		video.save(f'eval_{eval_mode}_{i}_{steps}.mp4')
...
def main(args):
	...
	eval_modes = ['color_easy', 'color_hard', 'video_easy', 'video_hard']
	...
	rewards = []
	for i, file in enumerate(os.listdir(model_dir)):
		agent = torch.load(os.path.join(model_dir, file))
		agent.train(False)

		print(f'\nEvaluating {work_dir} for {args.eval_episodes} episodes (mode: {args.eval_mode})')
		reward = evaluate(env, agent, video, args.eval_episodes, args.eval_mode, i)
