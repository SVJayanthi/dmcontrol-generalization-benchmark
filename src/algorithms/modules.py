...
class LUSRCompatibleSharedCNN(nn.Module):
    def __init__(self, obs_shape, weight_path):
        super().__init__()
        assert len(obs_shape) == 3
        self.num_layers = 11
        self.num_filters = 32
        ...
        self.preprocessing = nn.Sequential(CenterCrop(size=84), NormalizeImg())

        self.main = [nn.Conv2d(input_channel, self.num_filters, kernel_size, stride=2)]
        self.main.extend([nn.ReLU(), nn.Conv2d(self.num_filters, 2*self.num_filters, kernel_size, stride=2)])
        self.main.extend([nn.ReLU(), nn.Conv2d(2*self.num_filters, 4*self.num_filters, kernel_size, stride=1)])
        for _ in range(3, self.num_layers):
            self.main.append(nn.ReLU())
            self.main.append(nn.Conv2d(4*self.num_filters, 4*self.num_filters, kernel_size, stride=1))
        self.main = nn.Sequential(*self.main)

        if weight_path is None:
            self.apply(weight_init)
            print("Initialized LUSRCompatibleSharedCNN weights without pretraining")
        else:
            ...
            weights = {k.replace('main.', ''): v for k, v in weights.items()}
            for k in list(weights.keys()):
                if k not in self.main.state_dict().keys():
                    del weights[k]
            self.main.load_state_dict(weights)
            print("Loaded pretrained LUSRCompatibleSharedCNN weights")

        self.layers = nn.Sequential(self.preprocessing, self.main)
